...
async function bk_resolvePermission(uid, academyId, studentId) {
  const info = await getUserAcademyInfo(uid);
  if (!info || info.academyId !== academyId) {
    throw new HttpsError('permission-denied', 'Access denied: invalid academy');
  }
...
exports.cancelClassReservation = onCall(async (request) => {
  ...
  // Student-side cancel cutoff (staff bypass).
  if (!perm.isStaff && Number.isFinite(slotStartMillis)) {
    const deadline = slotStartMillis - cfg.cancelCutoffMinutes * 60 * 1000;
  ...
  // Pre-read the next waitlist candidate (promotion happens in the tx).
